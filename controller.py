...
class Index_Measure(app_manager.RyuApp):
        ...
        for stat in body:
            ...
            if stat.match['ip_proto'] == 1:
                ...
                if ev.msg.datapath.id not in self.port_nums.keys():
                    self.port_nums[ev.msg.datapath.id] = 0
                #计算端口数量
                self.port_nums[ev.msg.datapath.id]  += 1
    ...
    def _show_index(self):
        ...
        while True:
            ...
            for key, flows in self.flow_table.items():
                self.port_nums[key] = len(flows) / interval_time
            
            print "端口变化率", self.port_nums
